...
public class StringZh {
    public static void apply() {
        ...
        QH.string.处理midi音乐 = "处理MIDI音乐";
        QH.string.测试 = "测试";
        QH.string.处理midi音乐说明 = """
                \n开启此选项后支持部分32位游戏内播放midi音乐。如果担心会对其他游戏造成影响请关闭此选项。
                \n测试按钮：仅测试当前安卓环境是否可以正常播放midi音符。不代表实际游戏情况。
                \n已测试游戏：暴力摩托，大富翁4。（音频驱动选择PulseAudio）
                \n对wine的修改：启动容器时，若开启选项，会将特殊的 midimap.dll 从 apk/assets 解压到 c:\\windows\\sysywow64 ，\
                并添加环境变量 WINEDLLOVERRIDES=midimap=n 指定wine使用c盘的dll。若未开启选项，则环境变量为 midimap=b 。
                """;
    }
}
